...
val derived = Gen.boolean flatMap (b => if (b) Gen.unit(3) else Gen.unit(4)) // one could use map here
val list = derived.listOfN(Gen.choose(0, 3))
val lists = list.listOfN(10)

println(lists.sample.run(SimpleRNG(42)))
